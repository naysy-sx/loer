...
<?php $this->beginPage() ?>
    ...
    <script>
        $(document).on("click", ".save-order", function () {
            var client = $('.add-c-client_id').val();
            var court = $('.add-c-court_id').val();
            var ordernumber = $('.add-c-order-number').val();
            var instance = $('.add-c-instance').val();

            $.get("index.php?r=court-cases/add", {client, court, ordernumber, instance}, function (res) {
