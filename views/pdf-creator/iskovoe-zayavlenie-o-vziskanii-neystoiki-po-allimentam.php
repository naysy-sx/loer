...
<div class="title">
    ...
    <p>о взыскании неустойки по алиментам</p>
    <br>
</div>
<div class="main">
    <p><div class="redline"></div>&#171;<?=$data['delo_day']?>&#187; <?=$data['delo_month']?> <?=$data['delo_year']?> года судебным приставом-исполнителем <?=$data['delo_kem']?> возбуждено исполнительное производство № <?=$data['delo_number']?> о взыскании алиментов на содержание <?=$data['child_kto']?> в размере <?=$data['child_alim']?>.</p>
    <p><div class="redline"></div>Ответчик регулярно допускает просрочку выплаты алиментов с <?=$data['delo_year']?> года, уплачивая их нерегулярно. В настоящее время задолженность составляет <?=$data['delo_dolg']?> рублей.</p>
    <p><div class="redline"></div></p>
    <p><div class="redline"></div>В соответствии с п.2 ст. 115 СК РФ При образовании задолженности по вине лица, обязанного уплачивать алименты по решению суда, виновное лицо уплачивает получателю алиментов неустойку в размере одной десятой процента от суммы невыплаченных алиментов за каждый день просрочки.</p>
    <p><div class="redline"></div></p>
    <p><div class="redline"></div>В связи с этим, считаю, что Ответчик должен неустойку в размере 0,1% за каждый день просрочки. Расчет неустойки:</p>
    <p><div class="redline"></div></p>
...
<br>
    ...
    <p><div class="redline"></div>1. Взыскать с <?=$data['otvetchick_fio']?> в пользу <?=$data['istec_fio']?> неустойку за просрочку уплаты алименитов, в размере <?=$data['delo_summa']?>.</p>
<p><div class="redline"></div></p>
