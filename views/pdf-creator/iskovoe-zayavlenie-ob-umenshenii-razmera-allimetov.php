...
<div class="title">
    ...
    <p>об уменьшении размера алиментов</p>
    <br>
</div>
<div class="main">
    ...
    <p><div class="redline"></div>&#171;<?=$data['delo_day']?>&#187; <?=$data['delo_month']?> <?=$data['delo_year']?> года <?=$data['delo_kem']?> на основании <?=$data['delo_osnovanie']?> с меня в пользу Ответчика взыскиваются алименты на содержание ребенка в размере <?=$data['child_alim']?>.</p>
    <p><div class="redline"></div>С момента вынесения <?=$data['delo_vinesenie']?> изменились обстоятельства: <?=$data['delo_obstoyatelstva']?>, в следствие чего мое материальное положение значительно ухудшилось.</p>
...
<br>
    ...
    <p><div class="redline"></div>1. Уменьшить размер алиментов, взыскиваемых с меня <?=$data['delo_smenya']?> в пользу <?=$data['otvetchick_fio']?> по решению <?=$data['delo_poresheniy']?> по делу № <?=$data['delo_number']?> на содержание ребенка <?=$data['child_fio']?>, <?=$data['child_birth']?> года рождения до <?=$data['delo_do']?>.</p>
<p><div class="redline"></div></p>
