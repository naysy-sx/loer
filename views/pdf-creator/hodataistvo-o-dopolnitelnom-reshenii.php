...
<div class="title">
    <br>
    <p>Ходатайство</p>
    <p>о дополнительном решении</p>
    <br>

</div>
<div class="main">
    <p><div class="redline"></div>&#171;<?=$data['delo_day']?>&#187; <?=$data['delo_month']?> <?=$data['delo_year']?> года <?=$data['delo_sud']?> было вынесено решение по делу по иску <?=$data['delo_isk']?> к <?=$data['delo_k']?> о <?=$data['delo_o']?>, согласно которому <?=$data['delo_soderzhanie']?>.</p>
    <p><div class="redline"></div>Считаю, что решение суда является неполным и не отражает принятое решение по части требований, обозначенных в исковом заявлении, а именно: <?=$data['delo_nesoglasie']?>.</p>
...
<p><div class="redline"></div>1. Принять дополнительное решение по делу № <?=$data['delo_number2']?> по иску <?=$data['delo_isk']?> к <?=$data['delo_k']?> о <?=$data['delo_o']?>, которым разрешить следующие вопросы <?=$data['delo_voprosy']?>.</p>
<p><div class="redline"></div></p>
<p><div class="redline"></div></p>
